...
_langfuse = Langfuse(public_key=os.getenv('LANGFUSE_PUBLIC_KEY'),
...
def _ensure_prompt(name: str, prompt: str, model = os.getenv("AZURE_DEPLOYMENT"),temperature = 0.7)
    ...
    _LOGGER.info("prompt %s is created", name )

def register_prompt_once():
    _ensure_prompt(
        "Input_Guardrail",                   
        """ You are a guardrail checking the code and returning output in JSON format 
        your role is not to optimize the codee rather check whether the given code can 
        be optimize by other person or not. code: {{code}} 
        ...
        Return a JSON object with a 'code' field (true if conditions are met, false otherwise) and a 'condition' field (short condition in 10 words of which condition If False).
